...
// Removed: const path = require('path'); - since we're not dealing with local files
...
const RESTAURANT_PASS = 'zetsngkxipsergrm'; 
...
const db = mysql.createPool({
    host: 'localhost',
    user: 'root',
    password: '12345', // ⚠️ Update with your MySQL root password
    database: 'orders_db',
});
...
async function getProductId(productName, productType) {
    try {
        const { data } = await axios.get('http://localhost:5002/products/search', {
...
async function getCustomerDetails(customerId) {
    try {
        // 🚨 FIX: Correct AUTH_URL and endpoint
        const AUTH_URL = 'http://localhost:5004';
