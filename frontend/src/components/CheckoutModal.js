...
export default function CheckoutModal({
  ...
  useEffect(() => {
    async function fetchAddresses() {
      ...
      try {
        const response = await fetch(`http://localhost:5004/customer/${customerId}/addresses`);
  ...
  const handleDeleteAddress = async (idToDelete) => {
    try {
      const response = await fetch(`http://localhost:5004/customer/addresses/${idToDelete}`, {
        method: 'DELETE',
      });
