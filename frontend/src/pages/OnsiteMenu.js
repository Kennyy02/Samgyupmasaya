...
const OnsiteMenu = () => {
    ...
    const ORDER_SERVICE_URL = 'http://localhost:5003/orders/onsite';
    // URL to poll for all orders for the current table
    const ONSITE_ORDER_STATUS_URL = scannedTableId ? `http://localhost:5003/orders/onsite/status/${scannedTableId}` : null;
    ...
    const fetchOnsiteProducts = async () => {
        ...
        try {
            ...
            setError('Failed to load onsite menu. Please check the Product Service (Port 5002).');
    ...
    const handleOnsiteOrderSubmission = async (checkoutData) => {
        ...
        try {
            ...
            Swal.fire('Error', 'Failed to place order. Please check the Order Service (Port 5003).', 'error');
        }
    };
