...
const OnsiteMenu = () => {
    ...
    // =========================================================================
    // ⭐ THE CRITICAL FIX: Changed _SERVICE_URL to _API_URL to match Railway env variables
    // =========================================================================
    const PRODUCT_SERVICE_URL = process.env.REACT_APP_PRODUCT_API_URL || 'http://localhost:5002';
    ...
    const fetchOnsiteProducts = async () => {
        setLoading(true);
        try {
            // FIX VERIFIED: Now uses PRODUCT_SERVICE_URL which should be the Railway product API URL
    ...
    useEffect(() => {
        // NOTE: This call will now use the correct PRODUCT_API_URL
        fetchOnsiteProducts();
    }, []);
    ...
    useEffect(() => {
        ...
        if (scannedTableId && ONSITE_ORDER_STATUS_URL) {
            intervalId = setInterval(async () => {
                try {
                    // FIX VERIFIED: Now uses ORDER_SERVICE_BASE_URL which should be the Railway order API URL
    ...
    const handleOnsiteOrderSubmission = async (checkoutData) => {
        ...
        try {
            ...
            // FIX VERIFIED: Uses the correct ORDER_SERVICE_URL
                            ...
                            {paginatedProducts.length === 0 ? (
                                ...
                                paginatedProducts.map(product => (
                                    <div key={product.id} className="ol-item-card">
                                        <img
                                            // FIX VERIFIED: Image URL construction is correct
