...
    const ORDERS_BASE_URL = process.env.REACT_APP_ORDER_SERVICE_URL || 'http://localhost:5003/orders';
    const PRODUCTS_BASE_URL = process.env.REACT_APP_PRODUCT_SERVICE_URL || 'http://localhost:5002/products';
    // --- END OF UPDATE ---
...
            alert('Error fetching orders. Check if Order Service is running.');
        } finally {
            setLoading(false);
        }
    };
...
        try {
            // 1. Update the order status (Uses ORDERS_BASE_URL)
            await axios.put(`${ORDERS_BASE_URL}/online/${orderId}/status`, { status: newStatus });

            // 2. If the status is Delivered, update the product stock (Uses PRODUCTS_BASE_URL)
            if (isFinalStatus) {
...
        try {
            // First, update the order status (Uses ORDERS_BASE_URL)
            await axios.put(`${ORDERS_BASE_URL}/onsite/${orderId}/status`, { status: newStatus });

            // Second, call the product service to reduce the stock (Uses PRODUCTS_BASE_URL)
